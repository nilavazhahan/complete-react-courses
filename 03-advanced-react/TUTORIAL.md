...
#### FormData API 
 ...
 ->we can also submit the form with uncontrolled inputs, basically without referencing the state value. by useRef()
 ->how we can accomplish the same task with form Data API, which in my opinion is a better approach if you have more than one input as far as form data API.
   It's coming from Vanilla tJS and effectively it's a interface that allows us to construct. A set of key value pairs representing the form fields and their values.

  ->e.target refers to the DOM element that triggers an event.However, the e.currentTarget refers to the DOM element that eventlistener is listening on.
    So in our case, since the event "onSubmit" is set on a form. e.currentTarget returns our form element.
  ->why is that useful? Because in vanillaJS there's form data API, which is a setup how we can directly access the inputs we have as well as the values.

  ->object.fromEntries() Turns an array of arrays data structure into a object with key value pairs.
...
->It's a lot like "useState", so it preserves the value between the renders.
  But the difference is that updating useRef does not trigger re render.
  (when we work with useState() every time we update the value, we trigger re render.)

-> Very often useRef() is used to access DOM
-> Another way how we can use uncontrolled inputs and essentially that is using useRef hook.
...
-> custom hooks are great for Abstracting some functionality, basically to have less lines of code in the component.
   And as a result, it's going to be easier to manage our component.
   And also once we have custom hook, we can bring it from project to project, which of course speeds up our workflow.
...
-> how we can provide some data to all components in our application.
-> Any component that is going to be in the app where basically all our components, it will have access. And not only those components, but if I have any components inside of those components
...
-> cosnt [state, dispatch] = useReducer(reducerfn, defaultState)
    reducerFn ->This is where we will control essentially our entire state.
     we're getting back a "state" and a "dispatch function".
     state -> it's not just One state value.This is our entire state.
              You can have a bunch of values and all of them will be represented here with this state.Now, since this is an object, how we can access things in our application.
     dispatch function -> this function is updating the state.

     (dispatch function-> dispatches the action -> goes to reducer function -> then state gets update)
     You'll dispatch, you'll pass in the action basically what you want to do, and then it's going to go through the reducer and then whatever we get back here from the reducer, it's going to be our new state.

  To update the state, we should dispatch, then it will goto reducer then state gets update.

  You want to dispatch something called action, and that action is going to be handled in the reducer and then whatever gets returned from the reducer is going to be the new state. 

  const reducerFn = (state,action) => {}

  if we want to update something, if we want to change that state value, we go here with dispatch.
...
->So component is grabed into the memo function and effectively.It returns a memorized component. It just means that now the component will check whether the props have changed.
...
->every time the component is going to re render, function inside that component gets created from the scratch.But we are creating the function from scratch every time the state of the component gets updated.
...
-> useMemo memorizes the value that we're getting back from the function.So essentially, again, it remembers that value.
...
->we can use to treat some functionality as less urgent.
->useTransition just allows us to mark certain functionality as less urgent, which in turn prevents the whole UI blocking thing.

-> essentially something we want to keep fast, which is the response to our user, is still going to happen fast. However, The slow functionality is going to happen in the background.
...
-> how we can do code splitting in React, which in turn will allow us to progressively load or lazy load our application.

->the main benefits of such approach:
  1.Improve performance by splitting up a code into smaller, more manageable chunks.
    We can reduce the size of the initial JavaScript payload that needs to be loaded. This results in faster load times.
    And improved performance, especially on slow networks or low end devices.
  2.Better user experience with code splitting, only the essential code needed for the initial render of your application is loaded.
    The remaining code is loaded as the user interact with your application leading to a smoother, less blocking user experience.

the more code we send, the more time it will take for the browser to compile it, which in turn will affect how fast the user can interact with our application.
